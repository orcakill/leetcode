...
public class AppMovie {
    ...
    // 3-2  建立URL连接请求
    private static InputStream createInputStream(Object movieUrl, Integer avid) {
        InputStream inputStream = null;
        long start = System.currentTimeMillis();
        try {
            URL url = new URL((String) movieUrl);
    ...
    public static void downloadMovie(Integer avid, Integer cid) {
        ...
        Map dUrlMap = Collections.unmodifiableMap((Map) jsonArray.get(0));
        Object movieUrl = dUrlMap.get("url");
    ...
    public static Integer JsonGetCid(String cidJson) {
        ...
        Map pageMap = Collections.unmodifiableMap((Map) jsonArray.get(0));
    ...
    public static String createConnectionToJson(Integer avid) {
        ...
        //放完 movie地址
        return createConnection(cidUrl);
    }
