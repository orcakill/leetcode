...
public interface ImageOpenCVService {
	...
	static void imagesOpenCV (String folder, Double coefficient) throws InterruptedException, AWTException, IOException {
		int identificationAlgorithmType = 2;
		imagesOpenCV0 (folder, identificationAlgorithmType, "夜神模拟器", 60, 1, 2, true, true, coefficient);
	}
	
	static String imagesOpenCV (Map<String, String> files, Double coefficient, Integer start_num, Integer end_num) throws
	                                                                                                               InterruptedException,
	                                                                                                               AWTException,
	                                                                                                               IOException {
		int identificationAlgorithmType = 1;
		return imagesOpenCV0 (files, identificationAlgorithmType, "夜神模拟器", 60, start_num, end_num, true, true, coefficient);
	}
	
	static String imagesOpenCV0 (Map<String, String> files, int identificationAlgorithmType, String process, Integer re_num,
	                             Integer start_time, Integer end_time, Boolean boole, boolean isClick, Double coefficient) throws
	                                                                                                                       InterruptedException,
	                                                                                                                       AWTException,
	                                                                                                                       IOException {
		return ImageOpenCVServiceImpl.imagesClickBack (files, process, re_num, start_time, end_time, boole, isClick, coefficient);
	}
