    ...
    public void borderCheck(String process) throws IOException, InterruptedException, AWTException {
        ...
        while (!ImageService.imagesBack(region_JJTZJS, paramRGB(process, 3))) {
            ...
            if (!isBoundaryBreakthrough) {
                ...
                imagesBack(region_TCTZ, paramSIFT(process, 1, 10));
            }
            //判断能否选择个人结界
            ...
            if (booleanKXJJ) {
                ...
                log.info("判断是否有额外奖励");
                whetherAdditionalReward = imagesBack(region_TCTZ, paramSIFT(process, 1,5));
                if (whetherAdditionalReward) {
                    ImageService.imagesBack(region_TCTZ, paramSIFT(process,1,5));
                    log.info("领取额外奖励成功");
                } else {
                    log.info("没有额外奖励");
                }
