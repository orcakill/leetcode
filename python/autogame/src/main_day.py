    ...
    while True:
        ...
        if 0 <= current_hour < 5:
            if not task_list1[1]:
                ...
                OnmyojiController.create_execute_tasks(game_device, "2", "", game_id_all)
                task_list1[1] = True
        # 如果当前时间大于等于6点并且小于12点
            ...
            if (weekday == 3 and current_hour >= 9) or (weekday != 3):
                if not task_list2[1]:
                    ...
                    task_list2[1] = True
            logger.info("6-12,大号阴阳寮挑战循环")
            OnmyojiController.create_execute_tasks(game_device, "3", "", game_id_large)
        # 如果当前时间大于等于12点,小于17点
        elif 12 <= current_hour < 17:
            if current_hour < 17 and not task_list3[1]:
            ...
            if current_hour < 17 and not task_list3[3]:
                ...
                OnmyojiController.create_execute_tasks(game_device, "", "斗技", game_id_all, project_num_times=5)
                task_list3[1] = True
            if current_hour < 17 and not task_list3[4]:
                logger.info("12-17,大号小号,每日奖励")
                OnmyojiController.create_execute_tasks(game_device, "", "每日奖励", game_id_all)
                task_list3[4] = True
            if current_hour < 17 and not task_list3[3]:
                logger.info("12-17,大号，魂十一")
                OnmyojiController.create_execute_tasks(game_device, "", "魂十一", game_id_small, project_num_times=20)
                task_list3[3] = True
            if current_hour < 17 and not task_list3[4]:
                if weekday in [1, 6, 7]:
                    logger.info("12-17,周一、六、日，大号，日轮之陨")
                    OnmyojiController.create_execute_tasks(game_device, "", "日轮之陨", game_id_large)
                elif weekday in [2, 3, 4]:
                    logger.info("12-17,周二、三、四，大号，业原火")
                    OnmyojiController.create_execute_tasks(game_device, "", "业原火", game_id_large,
                                                           project_num_times=20)
                elif weekday in [5]:
                    logger.info("12-17,周五，大号，永生之海")
                    OnmyojiController.create_execute_tasks(game_device, "", "永生之海", game_id_large)
                task_list3[4] = True
            if current_hour < 17 and not task_list3[5]:
                logger.info("12-17,大号，个人突破")
                OnmyojiController.create_execute_tasks(game_device, "", "个人突破", game_id_large)
                logger.info("12-17,大号，式神寄养")
                OnmyojiController.create_execute_tasks(game_device, "5", "", game_id_large)
                task_list3[5] = True
        # 如果当前时间大于等于17点,小于24点
        elif 17 <= current_hour <= 23:
            if current_hour < 19 and not task_list4[1]:
                logger.info("17-24,17点,小号，式神寄养")
                OnmyojiController.create_execute_tasks(game_device, "", "式神寄养", game_id_small)
                task_list4[1] = True
            if current_hour < 22 and not task_list4[2]:
                logger.info("17-24,小号，逢魔之时")
                OnmyojiController.create_execute_tasks(game_device, "", "逢魔之时", game_id_small)
                task_list4[2] = True
