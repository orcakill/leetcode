...
class ComplexService:
    @staticmethod
    def broder_fight_end(fight_win: str, fight_fail: str, fight_again: str, fight_quit: str, timeout: int):
        """
        结界战斗，结束战斗
        1、战斗胜利,退出挑战
        2、退出挑战
        ...
        while time.time() - time_start < timeout:
            ...
            if is_win:
                ...
                return True
            is_quit = image_service.wait(fight_quit, timeout=1, is_click=True,interval=1)
            if is_quit:
                logger.debug("退出挑战")
                return True
            is_fail = image_service.wait(fight_again, timeout=1,interval=1)
            if is_fail:
                logger.debug("战斗失败")
