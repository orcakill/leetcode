...
def soul_arrange(game_task: []):
    ...
    for i in range(2):
        ...
        logger.debug("点击右侧详细，分辨率{}", resolution)
        ImageService.touch_coordinate((0.99 * resolution[0], 0.5 * resolution[1]), wait=5)
        logger.debug("点击右侧御魂")
        is_right_imperial_soul = ImageService.touch(Onmyoji.arrange_YCYH)
        if not is_right_imperial_soul:
            logger.debug("重新点击右侧详细")
            ImageService.touch_coordinate((0.99 * resolution[0], 0.5 * resolution[1]), wait=5)
            logger.debug("重新点击右侧御魂")
